...
function PendingShowingsCard() {

    const { data: count, isPending } = useQuery<number>({ queryKey: ['count-showings', "pending" ], queryFn: () => apiCountGetShowings( { status: "pending" } ) });
    ...
    return (
        <div className="border rounded-md p-6 w-full h-32 bg-white text-center flex flex-col justify-between items-center">
            ...
            <p className="text-gray-600">Pending schedules</p>
            <p className="text-black font-semibold text-lg font-[inter]">{count}</p>
        </div>
    )
}

export default PendingShowingsCard